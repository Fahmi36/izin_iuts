...
function AdministrasiPemohon() {
		...
		success:function(data) {
			if (data.success) {
				...
				if (data.rowCount > 0) {
					$('#idbangunan').val(localStorage.getItem("idbangunanuser"));
					$('#lat').val(lat);
					$('#long').val(lon);
					/* data administrasi pemohon*/
					$('#nama_pj').text(nama);
